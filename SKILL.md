...
- **TDD 铁律**: 不见见证不变更（Iron Law）
- **BDD 智能缺口**: 场景缺口自动扫描 + 用户选择
...
```
ai-engineer-workflow/
├── ai-engineer-workflow-v5-ulw/     # UltraWork 模式 (V5 完整版)
│   └── SKILL.md
├── ai-engineer-workflow-v5/         # V5 标准版
│   └── SKILL.md
...
| 3 Subagent | subagent-driven-development | Gate 3/4/5/6 + Loop 3/4 + 并行 + Iron Law |
...
❌ 跳过 Plan Agent → Process violation
...
记得，可以用plugin-loader和project-rules-generator来初始化项目，然后工作流是用来开发新需求的