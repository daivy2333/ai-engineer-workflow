...
**CodeGraph 增强**（如果项目已初始化 codegraph）:
- 用 `codegraph_search "{符号名}"` 快速定位符号位置
- 用 `codegraph_node "{符号名}"` 拉单个符号完整源码
- 用 `codegraph_callers "{符号名}"` 找上游调用
- 用 `codegraph_callees "{符号名}"` 找下游调用
- 用 `codegraph_impact "{符号名}"` 评估改动影响

**选择策略**:
- 简单符号位置 → grep learned.md 或 codegraph_search
- 需要调用链分析 → codegraph_callers/callees
- 需要完整源码 → codegraph_node
- 跨文件影响范围 → codegraph_impact
...
| 查阅 API | specs/learned/ → specs/references/ + codegraph_search | — |
...
| 查看变更 | openspec list | — |
| **CodeGraph 增强** | — | — |
| 理解模块工作原理 | codegraph_explore | — |
| 追踪调用链 | codegraph_callers/callees | — |
| 评估改动影响 | codegraph_impact | — |
| 定位符号位置 | codegraph_search | — |
...
与 CodeGraph 集成，代码知识优先用 codegraph_* 工具
```
...
**在 Pattern 4 (知识回忆) 中增加 CodeGraph 选项**:

```
查询策略（按优先级）:
  1. grep learned.md → 已有知识（最快）
  2. codegraph_search → 找符号位置（精确）
  3. codegraph_node → 拉单个完整源码
  4. codegraph_callers/callees → 调用链分析
  5. codegraph_impact → 改动影响评估
  6. Read + Grep → 兜底（仅当 CodeGraph 不可用）
```

### 与其他 Pattern 的协作

| Pattern | CodeGraph 配合 |
|---------|---------------|
| Pattern 4 知识回忆 | 优先用 codegraph_search 找符号 |
| Pattern 7 决策记录 | 引用 codegraph_impact 结果 |
| Pattern 8 学习记忆 | 反哺 API 路径时用 codegraph_search 验证 |
| Pattern 11 知识索取 | 提问前用 codegraph_* 探查，避免不必要的提问 |

### 知识反哺与 CodeGraph

```
知识反哺 (Pattern 8) 时的双重检查:
  1. 用 codegraph_search 验证 API/符号是否仍存在
  2. 用 codegraph_callers 确认调用关系
  3. 再写入 learned.md
```
...
❌ CodeGraph 可用时仍用 Read + Grep → CodeGraph 优先 violation
❌ 不验证 codegraph_status 就用 → 索引可能 stale
```
