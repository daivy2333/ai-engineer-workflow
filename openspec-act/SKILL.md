---
name: openspec-act
description: TDD执行、Gate验证、Review、归档收尾 — Phase 3-4 的实施工作流。由 openspec-plan 产出的计划驱动。
---

# OpenSpec Act — 执行与收尾工作流

> **前置依赖**：本 skill 假设 openspec-plan 已完成 Phase 1-2，存在经审批的 PLAN + OpenSpec 变更。
> 如未完成前置步骤，先调用 openspec-plan。
...
| 系统 | 角色 | 职责 | 激活时机 |
|------|------|------|----------|
| **本 workflow** | 主执行 | TDD循环 → Review → 验证 → 归档 | Phase 3-4 |
| **Karpathy** | 监察层 | 原则监察 | 全流程 |
| **BDD** | 缺口发现 | 智能扫描 + 用户选择 + 场景草图 | Phase 1（归 openspec-plan） |
...
- BDD — 本 skill 不激活（归 openspec-plan）
- TDD 负责**测试铁律**，Phase 3 强制执行
- OpenSpec 负责**变更管理**，Phase 3 apply，Phase 4 归档变更
- 本 skill 覆盖 Phase 3-4，Phase 1-2 由 openspec-plan 负责
...
- Gate 6 反射等决策点：用户先选一次策略 → Auto Mode 按策略自动继续
...
| Skill | 关系 | 说明 |
|-------|------|------|
| openspec-plan | 上游 | 本 skill 依赖 openspec-plan 产出的计划与变更 |
| openspec-assistant | 协作 | 日常文档维护，任务状态更新 |
