...
| 行为规格 | 验收过的系统当前行为，收尾时合并增量 | `.agents/specs/<domain>.md` |
...
Evidence 属于 change，不登记 R。普通验证结果只在 Act Response 保存不超过 20 行的决定性输出。只有用户要求、无法低成本复现、一次性环境即将消失、Incident/Blocker 现场或不可摘要的决定性结构才允许持久化；每个 Cycle 最多 5 个文件，整个 change 最多 20 个，禁止完整日志目录、源码副本和完整测试输出。Evidence 随 change 归档，不创建空占位目录。
...
- 验证结论在其覆盖范围未变化时可以采信复用。采信方做只读基线检查并注明来源；结论矛盾、验证不确定、覆盖不足或用户要求独立复现时重跑。重复运行相同材料不构成审计。
