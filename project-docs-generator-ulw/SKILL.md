...
额外rules约束：禁止把claude在git提交的时候列为coworker，不准设置共同创作者
---
...
**核心思想**：所有生成的文档必须结构化、关键词明确，便于 grep 快速检索。

### 结构设计原则

| 原则 | 实现 | grep 效果 |
|------|------|-----------|
| **标题层级清晰** | `##` / `###` 分节 | `grep "^##"` 快速定位章节 |
| **表格结构化** | `| 列1 | 列2 |` | `grep "| .*\|"` 定位表格行 |
| **关键词前置** | 条目名开头写关键词 | `grep "关键词"` 直接命中 |
| **日期格式统一** | `YYYY-MM-DD` | `grep -E "^\d{4}-\d{2}-\d{2}"` 定位时间线 |
| **分类标题明确** | `## API 路径` / `## 踩坑记录` | `grep "^## API"` 定位分类 |

### 具体实现要求

**learned.md**：
- API 路径表格：`| 名称 | 路径 | 用途 | 发现时间 |` — 关键词在"名称"列
- 踩坑档案：标题 `### [问题关键词]` — grep 标题即可定位
- 技巧模式：表格 `| 技巧关键词 | 适用场景 | 示例 |`

**architecture.md**：
- 决策条目：日期开头 `### 2024-01-15 - 决策关键词`
- 影响范围：模块名明确列出

**tasks.md**：
- 任务描述：关键词开头 `[关键词] 具体任务描述`
- 状态标记：`进行中` / `待办` / `阻塞` — grep 可过滤
...
```
❌ 模糊标题："一些问题" → grep 不知道搜什么
❌ 关键词埋在中间："关于 XXX 的说明" → grep "XXX" 可能漏掉
❌ 无结构段落：纯文本无标题 → grep 无法定位上下文
❌ 日期格式混乱："1月15日"、"Jan 15" → grep 无法统一匹配
```

### 验证 grep 友好性

生成文档后，执行以下 grep 命令验证可搜索性：

```bash
# 验证 learned.md 可搜索性
grep "^##" .claude/docs/learned.md          # 应返回所有分类标题
grep "^###" .claude/docs/learned.md          # 应返回所有踩坑档案标题
grep "| .*\| .*\|" .claude/docs/learned.md   # 应返回所有表格行

# 验证 architecture.md 决策时间线
grep -E "^\d{4}-\d{2}-\d{2}" .claude/docs/architecture.md  # 应返回所有日期

# 验证 tasks.md 状态
grep "进行中\|待办\|阻塞" .claude/docs/tasks.md  # 应返回所有状态标记
```
