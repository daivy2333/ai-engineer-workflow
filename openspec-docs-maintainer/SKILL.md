...
- 确定写入类型和目标文件。
- 复用当前会话中 Assistant 或上游 Skill 已读取且未变化的体系上下文，只补读支持本次写入所缺的信息。
...
- 行为规格：change 收尾时把 ADDED/MODIFIED/REMOVED 增量应用到 `openspec/specs/<domain>/spec.md`；域文件不存在则创建；合并冲突时停止并请求用户决定；archivist 归档的 change 不合并。
