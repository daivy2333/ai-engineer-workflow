...
**核心原则：CodeGraph 优先** — 探究时优先使用 CodeGraph 的 `codegraph_explore` 等工具，而不是 Read + Grep 组合。CodeGraph 是预建索引，比手动 Read 更快、更准、消耗更少。
...
**主入口：`codegraph_explore`** ⭐ — 大多数问题一次调用就能答完

| 工具 | 用途 | 何时用 |
|------|------|--------|
| **`codegraph_explore`** | 一次返回相关符号源码（按文件分组） | "X 怎么工作" / 流程追踪 / bug 定位 / 区域理解 |
| `codegraph_search` | 找符号位置（不带源码） | 只找位置，不带源码 |
| `codegraph_callers` | 谁调用了它 | 重构影响分析、debug 入口 |
| `codegraph_callees` | 它调用了谁 | 理解依赖、调用链追踪 |
| `codegraph_impact` | 改它会破坏什么 | 重构风险评估 |
| `codegraph_node` | 单个符号完整源码 | 同名重载时返回所有定义 |
| `codegraph_files` | 目录结构 | 比直接 ls 快 |
| `codegraph_status` | 索引健康/统计 | 排查问题 |

**调用守则**：
1. 先 `codegraph_explore`，别先 `grep` — codegraph 本身就是预建索引
2. 直接答，不要开 Explore 子 agent — 子 agent 会读文件，浪费
3. 信任结果 — 来自完整 AST 解析，比 grep 准
4. 一次 `codegraph_explore` 顶一组 `search`+`node` — 一次往返拿全部分组源码
5. 看到 ⚠️ stale banner 时，对那一个文件直接 Read，其它文件继续信任 codegraph
...
12. ⭐ CodeGraph 优先 — 探究时优先使用 CodeGraph（codegraph_explore 等），不要用 Read + Grep 组合。CodeGraph 是预建索引，更快更准
```
...
Step 1 — 项目扫描:
  ⭐ 使用 CodeGraph（如果项目已初始化）:
    1. codegraph_status → 检查索引健康状态
    2. codegraph_files → 获取目录结构（比 ls 快）
    3. codegraph_search → 查找核心符号（入口类/主函数）
  手动检查（如果 CodeGraph 不可用）:
    1. ls 项目根目录 → 了解项目结构
...
```
⭐ 核心协作：CodeGraph 是 explorer 的预建索引，比 Read + Grep 更快更准

协作场景:

1. 探究前检查 CodeGraph 状态:
   - codegraph_status → 确认索引 healthy
   - 未初始化时提示用户: codegraph init
   - 不可用时回退到 Read + Grep

2. 探究中用 CodeGraph 替代 Read:
   - codegraph_explore → 一次拿全部分组源码
   - codegraph_node → 拉单个完整源码
   - codegraph_files → 快速目录结构
   - codegraph_callers/callees → 调用链追踪
   - codegraph_impact → 改动影响评估

3. 探究产物可与 CodeGraph 互补:
   - .claude/analysis/ 文档 = 人工梳理的架构理解
   - CodeGraph = 机器索引的代码事实
   - 两者结合: 先用 CodeGraph 快速理解，再用分析文档深入

4. 委托给子 agent 时的提示词模板:
   "用 codegraph_explore 理解 {目标模块}，不要用 Read 文件。
    调用守则：
    - 先 codegraph_explore
    - 必要时 codegraph_node {具体符号}
    - 追踪调用链用 codegraph_callers/callees
    - 评估影响用 codegraph_impact"
```
...
⭐ CodeGraph 相关:
❌ CodeGraph 已初始化但用 Read + Grep → CodeGraph 优先 violation
❌ 先 grep 反向验证 CodeGraph 结果 → 浪费且不准
❌ 开 Explore 子 agent 读文件 → 浪费（CodeGraph 已建索引）
❌ 不用 codegraph_explore 一次拿全 → 多次 search+node 浪费
❌ 看到 ⚠️ stale banner 后对所有文件用 Read → 只对 banner 列出的文件用 Read
